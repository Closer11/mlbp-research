...
void NF_MLBPFormulation::createDecisionVariables(IloEnv env, const Instance<MLBP>& inst)
{
	...
	for (int k : inst.M) if (k > 0) {
		...
		for (int i : inst.B[k - 1])  // for every index in i, create an array of size j
			if (k == 1) f[k][i] = IloNumVarArray(env, inst.n[k], 0, 1, ILOBOOL);
	...
	SOUT() << "created " << num << " f_{ijk} variables" << std::endl;
}
...
void NF_MLBPFormulation::addConstraints(IloEnv env, IloModel model, const Instance<MLBP>& inst)
{
	...
	SOUT() << "added " << num << " constraints to enforce the max amount of flow" << std::endl;
	...
	sum.end();
	SOUT() << "added constraint to enforce the amount of demand in top-level bins" << std::endl;
	...
	num = 0; for (int k : inst.M) num += inst.n[k - 1];
