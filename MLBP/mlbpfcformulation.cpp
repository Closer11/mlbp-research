...
void MLBPFCFormulation::createDecisionVariables(IloEnv env, const Instance<MLBPFC>& inst)
{
	...
	// decision variables z_{rjk}
	z = IloArray<IloArray<IloNumVarArray>>(env, inst.m + 1);
	for (int k : inst.M) if (k > 0) {
		z[k] = IloArray<IloNumVarArray>(env, inst.q);
		for (int r = 0; r < inst.q; r++)
			z[k][r] = IloNumVarArray(env, inst.n[k], 0, 1, ILOBOOL);
	}
	num = 0; for (int k : inst.M) num += inst.q * inst.n[k];
	SOUT() << "created " << num << " z_{ijk} variables" << std::endl;
}

void MLBPFCFormulation::addConstraints(IloEnv env, IloModel model, const Instance<MLBPFC>& inst)
{
	...
	for (int i : inst.B[0])
		for (int j : inst.B[1])
			model.add(x[1][i][j] <= z[1][inst.g[i]][j]);
	
	for (int k : inst.M) if (k > 1)
		for (int j : inst.B[k])
			for (int i : inst.B[k - 1])
				for (int r = 0; r < inst.q; r++)
					model.add(z[k - 1][r][i] <= z[k][r][j] + (1 - x[k][i][j]));
}

void MLBPFCFormulation::addObjectiveFunction(IloEnv env, IloModel model, const Instance<MLBPFC>& inst)
{
	IloExpr sum(env);
	for (int k : inst.M)
		for (int j : inst.B[k])
	...
	for (int r = 0; r < inst.q; r++)
		for (int j : inst.B[inst.m])
			sum += z[inst.m][r][j] * inst.p;  // minimise the total penalty of all groups

	model.add(IloMinimize(env, sum)); sum.end();
	SOUT() << "added objective function" << std::endl;
}

void MLBPFCFormulation::extractSolution(IloCplex cplex, const Instance<MLBPFC>& inst, Solution<MLBPFC>& sol)
{
	...
	for (int r = 0; r < inst.q; r++)
		for (int j : inst.B[inst.m])
			if (cplex.getValue(z[inst.m][r][j]) > 0.5)
				sol.total_penalty += inst.p;
}
