...
#include "mlbpfcformulation.h"  // mip formulation for the multi-level bin packing problem with fragmentation constraints
...
int main(int argc, char* argv[])
{
	...
	try {
		...
		arg_parser.add<std::string>("prob", "Problem: Bin Packing (BP), Multi-Level Bin Packing (MLBP), Multi-Level Bin Packing with Fragmentation Constraints (MLBPFC)", "BP", {"BP", "MLBP", "MLBPFC", "APX"});
	...
	if (arg_parser.get<std::string>("prob") == "BP") {
		...
		if (status == MIPSolver<MLBP>::Feasible || status == MIPSolver<MLBP>::Optimal) {
			...
			SOUT() << "best dual bound value:\t" << sol.db << std::endl;
			SOUT() << "optimality gap:\t" << (double)(inst.objective(sol) - sol.db) / (double)inst.objective(sol) * 100.0 << "%" << std::endl;
			SOUT() << "representation:\t " << "Standard" << std::endl;
	...
	else if (arg_parser.get<std::string>("prob") == "MLBPFC") {
		/*****************************************************************************************/
		/** Multi-Level Bin Packing Problem with Fragmentation Constraints ***********************/
		/*****************************************************************************************/
		Instance<MLBPFC> inst(instance_filename);  // read MLBPFC instance

		SOUT() << "instance: " << instance_filename << std::endl;
		SOUT() << "\t" << inst << std::endl;

		Solution<MLBPFC> sol(inst);  // create empty MLBPFC solution

		// setup MIP solver
		MIPSolver<MLBPFC> mip_solver;
		mip_solver.setTimeLimit(arg_parser.get<int>("ttime"));  // set time limit; 0 -> no time limit
		mip_solver.setThreads(arg_parser.get<int>("threads"));  // number of used threads, should always be one for our experiments

		mip_solver.setFormulation<MLBPFCFormulation>();  // set MIP formulation

		/**************************************************************/
		auto status = mip_solver.run(inst, sol);  /** run MIP solver **/
		/**************************************************************/

		if (status == MIPSolver<MLBPFC>::Feasible || status == MIPSolver<MLBPFC>::Optimal) {
			SOUT() << std::endl;
			SOUT() << "# best solution:" << std::endl;
			SOUT() << "best objective value:\t" << inst.objective(sol) << std::endl;
			SOUT() << "best dual bound value:\t" << sol.db << std::endl;
		...
		// check if solution is feasible
		std::vector<std::string> msg;
		if (!SolutionVerifier<MLBPFC>::verify(inst, sol, &msg)) {
			std::cerr << "ERROR:" << std::endl;
			for (auto it = msg.begin(); it != msg.end(); ++it)
				std::cerr << *it << std::endl;
			return EXIT_FAILURE;
	...
	else if (arg_parser.get<std::string>("prob") == "APX") {
		/*****************************************************************************************/
		/** Approximation Algorithm for Multi-Level Bin Packing Problem **************************/
		/*****************************************************************************************/
		Instance<MLBP> inst(instance_filename);  // read MLBP instance
		...
		// setup AAF solver
		AAFSolver<FF> aaf_solver;
		aaf_solver.setTimeLimit(arg_parser.get<int>("ttime"));  // set time limit; 0 -> no time limit
		aaf_solver.setThreads(arg_parser.get<int>("threads"));  // number of used threads, should always be one for our experiments

		aaf_solver.setAlgorithm<FFAlgorithm>();  // set AAF Algorithm

		/**************************************************************/
		aaf_solver.run(inst, sol);  /** run AAF solver ****************/
		/**************************************************************/
